...
namespace GroupSpace23.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<GroupSpace23User> signInManager, UserManager<GroupSpace23User> userManager, ILogger<LoginModel> logger)
        {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.IsLockedOut)
                {
                    ...
                    // You might want to handle the lockout here or redirect to a lockout page
                    return RedirectToPage("./Lockout");
                }
                else
                {
                    _logger.LogWarning($"Invalid login attempt for user: {Input.UserName}");
